...
class Evaluator {
  ...
  def eval1[R](t : Term) : Option[Term] = {
    try {
      t match {
        ...
        case Lam(x, t1, t2) if eval1(t1.get) != None => Some(Lam(x, eval1(t1.get), t2)) //E-Abs2
        ...
        case IfThenElse(t1, t2, t3) if eval1(t1) != None => Some(IfThenElse(eval1(t1).get, t2, t3)) //E-If1
        case IfThenElse(t1, t2, t3) if eval1(t2) != None => Some(IfThenElse(t1, eval1(t2).get, t3)) // E-If2
        case IfThenElse(t1, t2, t3) if eval1(t3) != None => Some(IfThenElse(t1, t2, eval1(t3).get)) // E-If3
  ...
  def normalize(t : Term) : Term = eval(t)
}
